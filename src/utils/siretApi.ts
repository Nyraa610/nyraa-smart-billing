...
export async function fetchCompanyBySiret(siret: string): Promise<SiretCompanyData | null> {
  ...
  try {
    ...
    if (!data?.results || data.results.length === 0) {
      throw new Error(
        "Entreprise non trouvée. Elle est peut-être non-diffusible (micro-entreprise ayant exercé son droit d'opposition). Vous pouvez saisir les informations manuellement."
      );
    }
